...
/**
 * Base class for all HTTP-errors occurring when calling an {@link Endpoint}
 * method.
 */
@Getter
public class RestException extends EndpointException {
	...
	@Builder
	public RestException(int statusCode, String message, String responseBody, Throwable cause,
	...
	public RestException(int statusCode, String message, String responseBody) {
		super(message);
		this.statusCode = statusCode;
		this.responseBody = responseBody;
	}
	 ...
	 * @return An RestException that wraps up another exception caused by some
	 *         HTTP request error. This can be used by all services to translate
	 *         specific API exception into EndpointException. Notice input exception is returned if it is already a RestException.
	 */
	public static RestException fromHttpException(int statusCode, @NonNull Exception cause, String responseBody) {
		...
		case 500:
			return io.github.mzattera.predictivepowers.InternalServerException.internalServerExceptionBuilder() //
					...
					.responseBody(responseBody).build();
		default:
			return io.github.mzattera.predictivepowers.RestException.builder() //
